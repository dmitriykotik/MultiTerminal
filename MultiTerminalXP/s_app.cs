                                        ...
                                        try
                                        {
                                            if (File.Exists(input[3]))
                                            {
                                                ...
                                                if (Path.GetExtension(input[3]) == ".mpkg")
                                                {
                                                    ...
                                                    using (ZipFile zip = ZipFile.Read(input[3]))
                                                    {
                                                        foreach (ZipEntry n in zip)
                                                        {
                                                            n.Extract(Environment.CurrentDirectory + "\\tmp_packages");
                                                        }
                                                    }
                                                    if (File.Exists(Environment.CurrentDirectory + "\\tmp_packages\\package.data") && File.Exists(Environment.CurrentDirectory + "\\tmp_packages\\main.exe")) {
                                                        ...
                                                        using (ZipFile zip = ZipFile.Read(input[3]))
                                                        {
                                                            foreach (ZipEntry n in zip)
                                                            {
                                                                n.Extract(Environment.CurrentDirectory + "\\Packages\\" + File.ReadAllText(Environment.CurrentDirectory + "\\tmp_packages\\package.data"));
                                                            }
                                                        }
